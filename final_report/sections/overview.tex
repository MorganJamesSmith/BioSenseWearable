...
Data monitored by wearable tech can also be used to detect group illness rates. By using heart rate data and sleep data from Fitbit devices and cross referencing that data with the Centers for Disease Control and Prevention’s (CDC) Influenza Like Illness (ILI) data, a study was able to predict illness 6.3% to 32.9% better than previous methods [17]. 
...
%In order to reduce development cost and complexity, the hardware components of
%the project use sensors that are readily available from online sources, and 
%can be assembled on a PCB by hand relatively easily. As a result the 
%selection of sensors may be more limited or of lower quality than what is possible 
%for options that are made to order, or only available in large quantities had been
