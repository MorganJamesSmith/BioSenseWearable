...
to start the timer and stop the timer and store the current count was added in
places of interest. For each of the measured quantities 4096 samples where
recorded.
...
almost all of the samples where within a very small range with only a handful
...
While the average period between SysTick interrupts is slightly less then 1
millisecond, the count is very precise. All of the measured samples where within
750 microseconds of the mean value and 99\% of them where within 382 nanoseconds
of the mean which indicates that there is very little jitter in our millis
value.
...
1 millisecond sleeping. This indicates that the main interrupt source is the
SysTick timer that is used to generate interrupts every millisecond.
...
ADC sweeps where generally quite quick. In over 99\% of measurements the time
...
non-blocking operation is spread out across several iterations of the main
loop. The operation started, then later calls to the IMU service function
